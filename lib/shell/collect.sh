...
docker_collect_resource_telemetry() {
  ...
  runner="${DOCKER_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}/libexec/docker-telemetry-runner.py"
  [[ -f "$runner" ]] || { echo '{"sampled_at":null,"engine":{"available":false,"version":null},"items":[],"aggregates":{},"errors":[{"code":"collector_missing"}]}'; return 0; }
  python3 "$runner" 2>/dev/null || echo '{"sampled_at":null,"engine":{"available":false,"version":null},"items":[],"aggregates":{},"errors":[{"code":"collector_failed"}]}'
}
...
docker_collect_snapshot_json() {
  local generated_at hostname engine containers telemetry open watcher heartbeat label telemetry_enabled include_unlabeled interval incidents_dir
  ...
  incidents_dir="${DOCKER_WATCH_STATE_DIR:-/var/lib/docker-watch}/incidents"; open=0; [[ -d "$incidents_dir" ]] && open="$(find "$incidents_dir" -type f -name '*.json' 2>/dev/null | wc -l | tr -d ' ')"
...
containers.update({"monitored":len(items),"items":items,"without_memory_limit":int(aggregates.get('without_memory_limit') or 0),"without_healthcheck":int(aggregates.get('without_healthcheck') or 0)})
